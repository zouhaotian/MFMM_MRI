...
MRI.dat <- fread("dataset/Surv2.mri.dat.csv") %>% as.matrix()
...
fname = 'stan_fit/M1_whole_prior2.RData'
...
sign_beta_m = sign(beta_m) ## if sign is positive, we have to revert M
...
fitStan <- sampling(md, data = stan_dat, iter = n.iters, warmup = n.warmups, 
                    chains = 2, thin=1, init = inits, pars = pars, seed = 2021,
                    control = list(adapt_delta = 0.8, max_treedepth=10))
fname <- paste0('stan_fit/', 'M1_whole_posterior.RData')
save(list = 'fitStan', file = fname)

summ = summary(fitStan)$summary[1:(J*P + L0 + L1 + Lm + J-1 + J + n.tau + ncol(x) + L0 + L1*J + Lm), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'RData/summary_posterior_M1_whole.csv')

fname <- paste0('RData/summary_posterior_M1_whole.RData')
...
save(list = 'l', file = 'RData/summary_posterior_M1_whole_MP.RData')
