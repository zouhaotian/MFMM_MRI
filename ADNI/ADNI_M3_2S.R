...
fname = 'stan_fit/M3_2S_prior2.RData'
...
zeta1_sample <- extract(fitStan, pars = 'zeta_1')[[1]] 
zeta2_sample <- extract(fitStan, pars = 'zeta_2')[[1]] 
zeta3_sample <- extract(fitStan, pars = 'zeta_3')[[1]] 
zeta4_sample <- extract(fitStan, pars = 'zeta_4')[[1]] 
zeta5_sample <- extract(fitStan, pars = 'zeta_5')[[1]] 

zeta_1 = colMeans(zeta1_sample)
zeta_2 = colMeans(zeta2_sample)
zeta_3 = colMeans(zeta3_sample)
zeta_4 = colMeans(zeta4_sample)
zeta_5 = colMeans(zeta5_sample)
...
xi = summary.xi[, 1:L0]
mu_matrix = beta_m*(xi %*% f_l)
xi_m = m_mat - mu_matrix


## 
md = stan_model('source_code/M2_2S.stan')
stan_dat <- list(n = N, J = J, 
                 nobs = nrow(long), nmiss = Y_missing$len.missing, 
                 id_long = long$ID, 
                 L0 = L0, L1 = L1, Lm = Lm, 
                 ...
                 psi1_interval = psi1_interval, 
                 xi = t(xi), zeta_1 = zeta_1, zeta_2 = zeta_2, zeta_3 = zeta_3, 
                 zeta_4 = zeta_4, zeta_5 = zeta_5)
rnd <- 1/3
inits1 <- list(A1 = g.inits(mu_est[[1]]$coefficient), A2 = g.inits(mu_est[[2]]$coefficient),
               A3 = g.inits(mu_est[[3]]$coefficient), A4 = g.inits(mu_est[[4]]$coefficient), 
               A5 = g.inits(mu_est[[5]]$coefficient), 
...
fitStan <- sampling(md, data = stan_dat, iter = n.iters, warmup = n.warmups, 
                    chains = 2, thin=1, init = inits, pars = pars, seed = 2021,
                    control = list(adapt_delta = 0.8, max_treedepth=10))
fname <- paste0('stan_fit/', 'M3_2S_posterior.RData')
save(list = 'fitStan', file = fname)

summ = summary(fitStan)$summary[1:(J*P + Lm + J-1 + J + n.tau + ncol(x) + 1 + J + Lm), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'RData/summary_posterior_M3_2S.csv')

fname <- paste0('RData/summary_posterior_M3_2S.RData')
...
save(list = 'l', file = 'RData/summary_posterior_M3_2S_MP.RData')
