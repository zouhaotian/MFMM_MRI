...
fitStan <- sampling(md, data = stan_dat, iter = n.iters, warmup = n.warmups, 
                    chains = 2, thin=1, init = inits, pars = pars, seed = 2023,
                    control = list(adapt_delta = 0.8, max_treedepth=10))
fname = 'stan_fit/M2_prior1.RData'
save(list = 'fitStan', file = fname)
# load(fname)
summ = summary(fitStan)$summary[1:(J*P + L0 + L1 + J-1 + J + n.tau + ncol(x) + 1 + J), c(1, 3, 4, 8, 9, 10)]
...
save(list = 'l', file = 'RData/summary_prior_M2_MP.RData')
