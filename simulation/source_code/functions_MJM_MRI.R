...
calc_init_long1_without_B <- function(ID, Y, X.mat, time.spline, time){
  lme.obj <- lmer(Y ~ time.spline + X.mat +  (1+time |ID))
  fixed <- lme.obj@beta
  re <- as.matrix(ranef(lme.obj)$ID)
  cov.est <- unname(VarCorr(lme.obj)$ID)
  var.est <- c(diag(cov.est), sigma(lme.obj)^2)
...
calc_init_long2_without_B <- function(ID, Y, X.mat, time.spline, re){
  full.re <- expand_matrix(ID, re)
  lm.obj <- lm(Y ~ time.spline + X.mat + full.re)
  fixed <- unname(lm.obj$coefficients)
  sigma_est <- sigma(lm.obj)^2
  l <- list(fixed = fixed, var.est = sigma_est)
  return(l)
