...
H_true <- function(tp, x,
                   xi, zeta1, zeta2, zeta3, xi_m, t){
  rs_w = exp(tp$gamma01*xi[1] + tp$gamma02*xi[2] + 
             sum(tp$gamma11*zeta1) + sum(tp$gamma12*zeta2) + sum(tp$gamma13*zeta3))
  H <- exp(tp$logh0 + x*tp$gamma_x + sum(tp$gamma_m*xi_m))*rs_w*t
  return(H)
}
