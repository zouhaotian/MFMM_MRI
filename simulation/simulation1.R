...
source('source_code/data_gen2.R')
...
fname <- paste0('stan_fit1/', seed, '_prior2.RData')
...
#xi1_true = xi_true.train[ID1, ]; xi2_true = xi_true.train[ID2, ]
...
fitStan <- sampling(md, data = stan_dat, iter = n.iters, warmup = n.warmups, 
                    chains = 2, thin=1, init = inits, pars = pars, seed = 2020*seed,
                    control = list(adapt_delta = 0.8, max_treedepth=10))
fname <- paste0('stan_fit1/', seed, '_posterior.RData')
save(list = 'fitStan', file = fname)
# load(fname)
...
fname <- paste0('R1/', seed, '_posterior.RData')
...
fname <- paste0('R1/', seed, '_dp.RData')
