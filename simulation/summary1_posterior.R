...
for (i in 1:120){
  ...
  if (file.exists(fname) & file.exists(fname2)){
    ...
    kk = as.matrix(summary.parameters)
    d11[index.res, ] <- kk[3, ]
    beta_m[index.res] <- kk[11, 1]
...
summ.dat[1:(J+1+L0+L1+L0+Lm), 8] <- c(amse.mu1, amse.mu2, amse.mu3, amse.mum, 
                                      ...
                                      amse.psi_m[[5]])
write.csv(summ.dat, file = 'RData/summary1.csv')
...
write.table(AUC.true.matrix, file = 'RData/AUC_BS1.csv', append = FALSE, row.names = TRUE, col.names = TRUE, sep = ',')
write.table(AUC.matrix, file = 'RData/AUC_BS1.csv', append = TRUE, row.names = TRUE, col.names = FALSE, sep = ',')
write.table(BS.true.matrix, file = 'RData/AUC_BS1.csv', append = TRUE, row.names = TRUE, col.names = FALSE, sep = ',')
write.table(BS.matrix, file = 'RData/AUC_BS1.csv', append = TRUE, row.names = TRUE, col.names = FALSE, sep = ',')

write.table(integrated_AUC_BS, file = 'RData/integrated_AUC_BS1.csv', append = F, row.names = TRUE, col.names = TRUE, sep = ',')
