...
long.base = long.demean[which(long.demean[, 2]==0), ]
surv.train = cbind(surv.train, long.base[, 3:(2+J)])
x <- as.matrix(surv.train[, c(2, 5:(4+J))])
P_surv = ncol(x)

surv_time = surv.train$surv_time
status = surv.train$status
fit.cox <- coxph(Surv(surv_time, status) ~ x, method = 'breslow')
...
long.test.base = long.test[which(long.test[, 2]==0), ]
surv.test = cbind(surv.test, long.test.base[, 3:(2+J)])
...
for (Tstart in starting.time){
  ...
  x.test2 <- as.matrix(surv.test2[, c(2, 5:(4+J))])
  ...
  for (Tdelta in delta.time){
    ...
    for (i in 1:length(tmp.ID.test)){
      ...
      for (q in 1:Q){
        Hi_est_t0[q] <- H_est(x.test2[i, ], t = Tstart)
        Hi_est_t1[q] <- H_est(x.test2[i, ], t = Tstart + Tdelta)
      }
